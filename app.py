...
def load_properties_data():
    ...
    try:
        ...
        # ใช้ประเภททรัพย์ตามที่จัดกลุ่มไว้ใน excel เลย
        df['ประเภททรัพย์_กลุ่ม'] = df['ประเภททรัพย์']
...
/* ซ่อน Scrollbar ทุกเบราว์เซอร์ สำหรับหน้าหลักและ Tab แผนที่ */
html, body, #root, .stApp, div[data-testid="stAppViewContainer"], div[data-baseweb="tab-panel"]:first-of-type, section[data-testid="stMain"], .main, section.main {
    overflow: hidden !important;
    height: 100vh !important;
...
/* ซ่อนแถบ Scrollbar สำหรับทุกองค์ประกอบในเว็บ (รวมขวาสุด) */
* {
...
div[data-baseweb="tab-panel"]:first-of-type .stPlotlyChart, 
div[data-baseweb="tab-panel"]:first-of-type .stPlotlyChart > div, 
div[data-baseweb="tab-panel"]:first-of-type .stPlotlyChart .js-plotly-plot, 
div[data-baseweb="tab-panel"]:first-of-type .stPlotlyChart .plotly {
    margin-top: 0px !important;
    padding-top: 0px !important;
...
with tab1:
    with st.container(key="tab_map"):
        ...
        if map_data.empty:
            ...
            st.plotly_chart(fig_map, use_container_width=True, theme=None, config={"scrollZoom": True, "responsive": True})
    ...
    if st.button("🔍 เริ่มการวิเคราะห์เปรียบเทียบพิกัด", type="primary"):
        with st.spinner("กำลังค้นหาทำเลและทรัพย์ที่อยู่รอบๆ..."):
            ...
            if nearby_df.empty:
                ...
                st.plotly_chart(fig_compare, use_container_width=True, theme=None, config={"scrollZoom": True, "responsive": True})
