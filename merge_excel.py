...
def merge_all_excel():
    ...
    files = {
        "Baania": base_dir / "Baania NPA new" / "Baania NPA.xlsx",
        ...
        "DDproperty": base_dir / "DDproperty NPA" / "DDproperty NPA.xlsx",
        "Taladnudbaan": base_dir / "Taladnudbaan NPA" / "Taladnudbaan NPA.xlsx",
    }
    ...
    for company, path in files.items():
        ...
        try:
            ...
            # ปรับจูนโครงสร้างคอลัมน์และข้อมูลสำหรับ DDproperty
            if company == "DDproperty":
                # Rename columns (ลบช่องว่างหน้าวงเล็บออกเนื่องจากในไฟล์ไม่มีช่องว่าง)
                rename_map = {
                    "พื้นที่(ไร่-งาน-วา)": "พื้นที่ (ไร่-งาน-วา)",
                    "พื้นที่ใช้สอย(ตร.ม.)": "พื้นที่ใช้สอย (ตร.ม.)"
                }
                df = df.rename(columns=rename_map)
                
                # ทำความสะอาดข้อมูลราคา (เช่น ฿ 12,900,000 - ฿ 12,900,000 หรือมีเครื่องหมายเงินตราอื่นๆ)
                def clean_dd_price(val):
                    if pd.isna(val) or not isinstance(val, str):
                        return val
                    # เอาสัญลักษณ์เงินบาท คอมม่า และตัดช่องว่างออก
                    ...
                    if "-" in val_clean:
                        ...
                        try:
                            ...
                            return str(int((p1 + p2) / 2)) # ใช้ราคาเฉลี่ย
                        except:
                            return parts[0].strip()
                    return val_clean
                
                if "ราคา" in df.columns:
                    df["ราคา"] = df["ราคา"].apply(clean_dd_price)
                    
            # ปรับจูนโครงสร้างคอลัมน์สำหรับ Taladnudbaan
            if company == "Taladnudbaan":
                rename_map = {
                    "พื้นที่(ไร่-งาน-วา)": "พื้นที่ (ไร่-งาน-วา)",
                    "พื้นที่ใช้สอย(ตร.ม.)": "พื้นที่ใช้สอย (ตร.ม.)"
                }
    ...
    # ทำความสะอาดและจัดกลุ่มมาตรฐานของ 'ประเภทการขาย' (Sale Type Cleaning)
    def clean_sale_type_val(val):
        ...
        # 4. กลุ่มประมูล / ขายทอดตลาด (Auction)
        if v_lower in ["auction", "ประมูล", "ขายทอดตลาด"]:
            return "ประมูล / ขายทอดตลาด"
            
        # 5. กลุ่มขายดาวน์ / รอประกาศ (Down Payment / Pending)
        if v_lower in ["down-payment", "ขายดาวน์", "รอประกาศราคา"]:
            return "ขายดาวน์ / รอประกาศ"
            
        return v

    combined_df["ประเภทการขาย"] = combined_df["ประเภทการขาย"].apply(clean_sale_type_val)
